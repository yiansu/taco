...
void HEARTBEAT_loop2(int D2_dimension, int A2_dimension, int C2_dimension, double *A_vals, double *B_vals, double *C_vals, double *D_vals, uint64_t i, uint64_t k, uint64_t lB, uint64_t l);
...
sliceTasksPointer slice_tasks[3] = {
...
leftoverTasksPointer leftover_tasks[3] = {
...
void HEARTBEAT_loop1(int *B3_pos, int *B3_crd, int D2_dimension, int A2_dimension, int C2_dimension, double *A_vals, double *B_vals, double *C_vals, double *D_vals, uint64_t i, uint64_t kB, uint64_t k) {
  for (uint64_t lB = B3_pos[kB]; lB < B3_pos[(kB + 1)]; lB++) {
    ...
    HEARTBEAT_loop2(D2_dimension, A2_dimension, C2_dimension, A_vals, B_vals, C_vals, D_vals, i, k, lB, l);
  }
}
...
int64_t HEARTBEAT_loop1_slice(uint64_t startIter, uint64_t maxIter, uint64_t *cxts, uint64_t *constLiveIns, uint64_t myIndex, task_memory_t *tmem) {
  ...
  for (; startIter < maxIter; startIter++) {
    ...
    rc = HEARTBEAT_loop2_slice((uint64_t)0, (uint64_t)D2_dimension, cxts, constLiveIns, 0, tmem);
...
int64_t HEARTBEAT_loop2_slice(uint64_t startIter, uint64_t maxIter, uint64_t *cxts, uint64_t *constLiveIns, uint64_t myIndex, task_memory_t *tmem) {
  ...
  for (; startIter < maxIter; startIter += chunksize) {
...
#endif
      cxts[(LEVEL_TWO-offset) * CACHELINE + START_ITER] = low - 1;
      rc = loop_handler(
        cxts, constLiveIns, LEVEL_TWO, NUM_LEVELS, tmem,
    ...
    if(unlikely(__rf_test())) {
      cxts[(LEVEL_TWO-offset) * CACHELINE + START_ITER] = low - 1;
      __rf_handle_wrapper(
        rc, cxts, constLiveIns, LEVEL_TWO, NUM_LEVELS, tmem,
        slice_tasks, leftover_tasks, &leftover_selector
      );
      if (rc > 0) {
        break;
